...
export const usePlaylistStore = create((set, get) => ({
  ...
  getPlaylistDetails: async (playlistId) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/playlist/${playlistId}/playlistById`
      );
  ...
  addProblemToPlaylist: async (playlistId, problemIds) => {
    try {
      ...
      await axiosInstance.post(
        `/playlist/${playlistId}/addProblem/${problemIds}`,
        {
          problemIds,
        }
      );
  ...
  removeProblemFromPlaylist: async (playlistId, problemIds, problemId) => {
    try {
      set({ isLoading: true });
      await axiosInstance.post(
        `/playlist/${playlistId}/removeProblem/${problemId}`,
        {
          problemIds,
        }
      );
